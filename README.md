<h1 align="center">
  <a href="https://willfinger.com">
    <img src="https://readme-typing-svg.herokuapp.com?font=Space+Grotesk&size=36&duration=3000&pause=1000&color=8B5CF6&center=true&vCenter=true&width=600&lines=William+Finger" alt="William Finger"/>
  </a>
</h1>

<p align="center">
  <em>Principal Product Designer & AI Architect</em>
</p>

<p align="center">
  <a href="https://willfinger.com"><img src="https://img.shields.io/badge/Portfolio-willfinger.com-8B5CF6?style=for-the-badge"/></a>
  <a href="https://www.linkedin.com/in/willfinger/"><img src="https://img.shields.io/badge/LinkedIn-William%20Finger-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/></a>
  <a href="mailto:william@willfinger.com"><img src="https://img.shields.io/badge/Email-william%40willfinger.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/></a>
</p>

---

### 👋 About Me

I love to architect **digital ecosystems**. Based in **Lisbon, Portugal** 🇵🇹, with 10+ years designing products across three continents.

> *"I don't just design interfaces—I live them."*

---

### 📊 Impact by the Numbers

<table align="center">
  <tr>
    <td align="center" width="25%">
      <img src="https://img.shields.io/badge/%24100M+-TVL%20Added-2ea44f?style=for-the-badge"/>
      <br/><sub><b>DeFi Liquidity</b></sub>
    </td>
    <td align="center" width="25%">
      <img src="https://img.shields.io/badge/150M+-Users%20Served-blue?style=for-the-badge"/>
      <br/><sub><b>Global Reach</b></sub>
    </td>
    <td align="center" width="25%">
      <img src="https://img.shields.io/badge/380K+-DAO%20Voters-purple?style=for-the-badge"/>
      <br/><sub><b>Largest NEAR Vote</b></sub>
    </td>
    <td align="center" width="25%">
      <img src="https://img.shields.io/badge/4%20%E2%86%92%20150+-Team%20Scale-orange?style=for-the-badge"/>
      <br/><sub><b>Org Growth</b></sub>
    </td>
  </tr>
</table>

---

### 🎯 Core Focus

```typescript
const william = {
  location: "Lisbon, Portugal 🇵🇹",
  role: "Principal Product Designer & AI Architect",

  currentFocus: [
    "DeFi Protocols",
    "AI Agents & LLM Interfaces",
    "Yield Systems"
  ],

  disciplines: {
    strategic: ["Revenue Optimization", "Retention Mechanics", "Tokenomics Design", "CRO"],
    technical: ["Web3 Governance", "DeFi/FinTech Architecture", "ZK Applications"],
    leadership: ["Design Operations", "Team Scaling", "Cross-Cultural Management"]
  },

  philosophy: "Complexity kills conversion. I ruthlessly eliminate features that don't drive the core metric."
};
```

---

### 🚀 Featured Projects

| Project | Challenge | Impact |
|---------|-----------|--------|
| **[Sweat Economy](https://willfinger.com/case-study/sweat-economy)** | Migrate 150M+ Web2 users on-chain | Record-breaking DAO governance |
| **[Astraly Labs](https://willfinger.com/case-study/astraly-labs)** | Launch ZK Rollup from 0 → 30K users | Strategic partnerships secured |
| **[Tomb Finance](https://willfinger.com/case-study/tomb-finance)** | Recover de-pegged stablecoin | **$100M+ TVL recovered** |
| **[PremierSoft](https://willfinger.com/case-study/premiersoft)** | Scale design operations | 4 → 150+ team members |

🔗 **View all case studies at [willfinger.com](https://willfinger.com)**

---

### 💼 Career Timeline

```
2023 - 2025  ─── Sweat Economy     ─── Principal Product Designer
2022         ─── Astraly Labs      ─── Principal Product Designer
2021         ─── Tomb Finance      ─── Principal Product Designer
2016 - 2022  ─── PremierSoft       ─── Senior Product Designer
```

---

### 🛠️ Tech Stack

<p align="center">
  <img src="https://img.shields.io/badge/Figma-F24E1E?style=flat-square&logo=figma&logoColor=white"/>
  <img src="https://img.shields.io/badge/React-20232A?style=flat-square&logo=react&logoColor=61DAFB"/>
  <img src="https://img.shields.io/badge/Tailwind-38B2AC?style=flat-square&logo=tailwind-css&logoColor=white"/>
  <img src="https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white"/>
  <img src="https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white"/>
  <img src="https://img.shields.io/badge/After%20Effects-9999FF?style=flat-square&logo=adobe-after-effects&logoColor=white"/>
</p>

---

### 🏆 Recognition

| Award | Organization | Year |
|-------|--------------|------|
| 🏅 Indispensable Employee Award | PremierSoft | 2021 |
| 🥇 1st Place | Playbor National Competition | 2017 |
| 🥇 1st Place | SENAI Game Jam | 2014 |

---

### 📬 Let's Connect

<p align="center">
  I'm here to make your company thrive.
</p>

<p align="center">
  <a href="mailto:william@willfinger.com">
    <img src="https://img.shields.io/badge/Get%20in%20Touch-william%40willfinger.com-8B5CF6?style=for-the-badge"/>
  </a>
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=willbnu&color=8B5CF6&style=flat-square"/>
</p>
